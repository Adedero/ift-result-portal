...
const { uploadFile, deleteFiles } = require("../../../utils/upload-file");
...
const GeneralController = {
  ...
  uploadResult: async (req, res) => {
    ...
    try {
      ...
      //Upload to firebase using filename;
  ...
  changeImage: async (req, res) => {
    ...
    const randomString = generateRandomPin(8, "alphanumeric")
    const fileName = `${user._id}-${user.firstName.toLowerCase()}-${user.lastName.toLowerCase()}-${randomString}.${ext}`;
    const uploadDir = path.resolve("public/users");
    const filePath = path.join(uploadDir, fileName);

    if (user.imageName) {
      await deleteFiles(user.imageName, { path: "ift/users" });
    }
    ...
    const edits = { imageName: fileName, image: url }
    await db.User.updateOne({ _id: id }, { $set: edits })
