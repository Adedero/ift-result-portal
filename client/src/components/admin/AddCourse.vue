...
const submitForm = async () => {
  ...
  await useFetch(
    "admin/course",
    { router, toast, method: "POST", body: { course: course.value }, toastOnFailure: true, toastLife: 5000 },
    (payload) => {
      emit("add-course", payload.course)
    }
  );
  loading.value = false;
}
  ...
  <div class="min-w-[min(70dvw,20rem)] md:w-[30rem] lg:w-[36rem]">
    ...
    <div class="mt-4 grid md:grid-cols-2 lg:grid-cols-3 gap-4 pb-8">
      ...
      <div class="flex items-end md:order-5">
        <Button label="Submit" icon="pi pi-file-plus"
