...
const links = ref([
  {
    heading: "General",
    items: [
      { id: 1, title: "Results", icon: "pi pi-book", name: "staff-results", params: {} },
    ]
  },
  {
    heading: "Settings",
    items: [
      { id: 1, title: "Biometrics", icon: "pi pi-eye", name: "staff-biometrics", params: {} },
      { id: 2, title: "Profile", icon: "pi pi-user", name: "staff-profile", params: {} },
    ]
  }
])
</script>
      ...
      <div class="h-full overflow-y-auto w-full border-r relative">
        <header class="pt-2 pb-4 flex flex-col items-center justify-center">
          ...
          <div class="text-center">
            ...
            <p class="text-sm">Staff ID: {{ userStore.user?.staffId ?? '' }}</p>
            <p class="text-sm">Class: {{ userStore.user?.studentClass ?? '' }}</p>
            <p class="text-xs uppercase text-red-500">STAFF</p>
